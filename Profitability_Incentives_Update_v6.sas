/* This is the connection string to ORACLE */

%LET MyId=HMA03468;
%LET SalesPW = U@@ex!frz2;/*M!dex@mjz8*/
%LET QdbPW = Pass#bnnh2;/*M!dex@mjz8*/

	
libname SALES oracle user="&MyId." password="&SalesPW." path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
(SERVICE_NAME=PDWH)))" schema=HMA_DM_SALES;

/*libname PROD oracle user="&MyId." password="&SalesPW." path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
(SERVICE_NAME=PDWH)))" schema=HMA_PROD;

libname SERVICE oracle user="&MyId." password="&SalesPW." path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
(SERVICE_NAME=PDWH)))" schema=HMA_DM_SERVICE;*/

libname WJSAS '/sashome/prod/HMA03468/2_SASLIB';
libname RV '/sashome/prod/HMA03468/4_SASProjects/RV';
libname LOYAL '/sashome/prod/HMA03468/2_SASLIB';
libname BUDGET '/sashome/prod/HMA03468/2_SASLIB/Budget';
libname HAEA '/sashome/prod/HMA03468/AA_HAEA';
libname PROFIT '/sashome/prod/HMA03468/2_SASLIB/Profitability_Model';
*libname OPTI '/sashome/prod/sasbatch/users/walter';
libname PINDATA '/sashome/prod/HMA03468/2_SASLIB';
libname HMASAS '/sashome/prod/sasbatch/users/walter';
/*libname HMASAP '/sashome/prod/sasbatch/users/walter/SAPdata';*/
libname ALG '/sashome/prod/HMA03468/2_SASLIB/ALG';
libname PINWEEK '/sashome/prod/HMA03468/2_SASLIB/PIN_Weekly';
libname HMFBILL '/sasshare/HMF_Billing';
libname MOD_INT '/sashome/prod/sasbatch/users/incentives_process/modeling_interm_tables/';
libname main "/sasshare/hpo/main/";
/*libname TEMPMAU '/sashome/prod/mauricio.alvare/Temp_outputs';*/

libname INCENT oracle user="&MyId." password="&SalesPW." path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
(SERVICE_NAME=PDWH)))" schema=HMA_DM_INCENTIVES;

/*libname CUST oracle user="&MyId." password="&SalesPW." path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
(SERVICE_NAME=PDWH)))" schema=HMA_ODS;*/

/*This is a different PW and needs reset at PHQDB  Saxx!njk8 */

/*libname PRO_ODS oracle user=HMA03468 password="&QdbPW" path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
(SERVICE_NAME=PHQDB)))" schema="HMA_ODS";

libname lookups oracle user=HMA03468 password="&QdbPW" path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
(SERVICE_NAME=PHQDB)))" schema="HMA_PROD_LOOKUP";*/
libname QDB_MDM oracle user=HMA03468 password="&QdbPW" path="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)
	(HOST=nasipexadb-scan)(PORT=1521)(LOAD_BALANCE=yes))(CONNECT_DATA=(SERVER=dedicated)
	(SERVICE_NAME=PHQDB)))" schema="HMA_MDM";
...
/*Here's where we start bringing in the imported data */


/*data work.sstdins (keep=txtMY Model_Code);
set work.std_ins;
run;
proc sort data=work.sstdins
noduprecs;
by txtMY Model_Code;
run;

PROC SORT DATA=work.std_ins
  NODUPRECS ;
 BY TxtMY Model_Code ;
RUN ;
...
PROC SQL;
   CREATE TABLE PROFIT.STD_INS AS 
   SELECT DISTINCT t1.TxtMy, 
          t1.Model_Code, 
          t1.Curr_Std, 
          t1.Eff_1_1_17
      FROM WORK.STD_INS t1;
QUIT;*/


/*PROC SORT DATA=work.stduty
  NODUPRECS ;
 BY Model_Year Model_Code ;
RUN ;
PROC SQL;
   CREATE TABLE PROFIT.Duty1 AS 
   SELECT DISTINCT put(t1.Model_Year,w4.) as TxtMY, 
          t1.Model_Code, 
          t1.DutyStd
      FROM WORK.STDUTy t1;
QUIT;*/
/*PROC SORT DATA=work.holder
  NODUPRECS ;
 BY Model_Year Model_Code port;
RUN ;
PROC SQL;
   CREATE TABLE work.MFT1 AS 
   SELECT DISTINCT t1.Model_Year as TxtMY, 
          t1.Model_Code, 
          input(t1.MFT_Std, comma12.) as MFT ,
          t1.Port
      FROM WORK.holder t1;
QUIT;
data work.mft2 (drop=MFT);
set work.mft1;
MFT_STD = MFT;
run;*/

/*PROC SORT DATA=work.std_ppr
  NODUPRECS ;
 BY Model_Year Model_Code port;
RUN ;

PROC SQL;
   CREATE TABLE PROFIT.PPR AS 
   SELECT DISTINCT t1.Model_Year as TxtMY, 
          t1.Model_Code, 
          t1.PPR_STD, 
          t1.Port
      FROM WORK.STD_PPR t1;
QUIT;*/
/*PROC SORT DATA=work.std_rft
  NODUPRECS ;
 BY Model_Year Model_Code port;
RUN ;
PROC SQL;
   CREATE TABLE PROFIT.RFT1 AS 
   SELECT DISTINCT t1.Model_Year, 
          t1.Model_Code, 
		  put(t1.Model_Year,BEST4.) as TxtMY,
          /* MAX_of_RFT_STD */
          /*(MAX(t1.RFT_STD)) FORMAT=COMMA12.2 AS MAX_of_RFT_STD, 
          t1.Port
      FROM WORK.STD_RFT t1
	  WHERE t1.MODEL_YEAR is not MISSING
      GROUP BY t1.Model_Year,
               t1.Model_Code,
               t1.Port;
QUIT;*/
/*PROC SORT DATA=work.std_acc
  NODUPRECS ;
 BY Model_Year Model_Code acc_code;
RUN ;
PROC SQL;
   CREATE TABLE work.Accessory1 AS 
   SELECT DISTINCT t1.Model_Year as TxtMY, 
          t1.Model_Code, 
          t1.ACC_Code, 
          /* ACC_COST */
    /*     (MAX(t1.ACC_Cost1)) FORMAT=NEGPAREN12. AS ACC_COST
      FROM WORK.STD_ACC t1
      GROUP BY t1.Model_Year,
               t1.Model_Code,
               t1.ACC_Code;
QUIT;*/
/*PROC SORT DATA=work.std_fob
  NODUPRECS ;
 BY txtMY Model_Code ;
RUN ;
data work.std_fob2;
set work.std_fob;
BY txtMY Model_Code ;
fob_std = input(FOB_STD, 15.2);
run; */

/*PROC SQL;
   CREATE TABLE PROFIT.STD_PRICE AS 
   SELECT DISTINCT t1.TxtMy, 
          t1.Model_Code, 
          max(t1.FOBSTD2) as FOBSTD3, 
          /* DutyDollar */
        *    max((t1.FOBSTD2*t3.DutyStd)) FORMAT=12.2 AS DutyDollar, 
          /* BEF2017 */
            max((t1.FOBSTD2 * t2.Curr_std)) FORMAT=12.2 AS BEF2017, 
          /* AF2017 */
            max((t1.FOBSTD2 * t2.Eff_1_1_17)) FORMAT=12.2 AS AF2017
      FROM WORK.STD_FOB2 t1
           LEFT JOIN PROFIT.STD_INS t2 ON (t1.TxtMY = t2.TxtMY) AND (t1.Model_Code = t2.Model_Code)
           LEFT JOIN PROFIT.DUTY t3 ON (t1.TxtMY = t3.TxtMY) AND (t1.Model_Code = t3.Model_Code)
		GROUP BY t1.TxtMY, t1.Model_Code;
/*QUIT;*/
/*PROC SORT DATA=work.std_pio
  NODUPRECS ;
 BY Model_Year Model_Code PIO_Code Port;
RUN ;



PROC SQL;
   CREATE TABLE PROFIT.PIO AS 
   SELECT DISTINCT 
   /* TxtMY */
      *   (put(t1.Model_Year,best4.)) AS TxtMY, 
          t1.Model_Code, 
          t1.PIO_Code, 
          /* PIO_MAX */
            (MAX(t1.Total_PIO)) FORMAT=NEGPAREN12.2 AS PIO_MAX, 
          t1.Port
      FROM WORK.STD_PIO t1
      GROUP BY (CALCULATED TxtMY),
               t1.Model_Code,
               t1.PIO_Code,
               t1.Port;
/*QUIT;
PROC SORT DATA=PROFIT.PIO
  NODUPRECS ;
 BY TxtMY Model_Code PIO_Code Port;
RUN ;*/
...
PROC SQL;
   ...
   SELECT DISTINCT t1.TxtMY, 
      ...
      FROM WORK.SYS_ACC_CODE t1
           LEFT JOIN PROFIT.ACCESSORY t2 ON (t1.TxtMY = t2.TxtMY) AND (t1.MODEL_CD = t2.Model_Code) AND (t1.ACCCD = 
          t2.ACC_Code)
      GROUP BY t1.TxtMY,
               t1.MODEL_CD,
               t1.ACCESSORY_GROUP_CD;
...
PROC SQL;
   ...
   SELECT t1.VEHICLE_KEY, 
      ...
      WHERE t1.WHOLESALE_DATE >= '1Jan2013:0:0:0'dt
...
PROC SQL;
   CREATE TABLE HMASAS.SALES_OBJECT AS 
   SELECT t1.OBJECTIVE_YEAR, 
          t1.SALES_TYPE, 
          t1.SALES_PLAN_NUM, 
          t1.OBJECTIVE_TYPE, 
          t1.REGION_CD, 
          ...
          t2.ADI_DESC, 
          t1.DEALER_CD, 
          /* SUM_of_JAN_OBJ_UNITS */
            (SUM(t1.JAN_OBJ_UNITS)) AS JAN_OBJ, 
          /* SUM_of_FEB_OBJ_UNITS */
            (SUM(t1.FEB_OBJ_UNITS)) AS FEB_OBJ, 
          /* SUM_of_MAR_OBJ_UNITS */
            (SUM(t1.MAR_OBJ_UNITS)) AS MAR_OBJ, 
          /* SUM_of_APR_OBJ_UNITS */
            (SUM(t1.APR_OBJ_UNITS)) AS APR_OBJ, 
          /* SUM_of_MAY_OBJ_UNITS */
            (SUM(t1.MAY_OBJ_UNITS)) AS MAY_OBJ, 
          /* SUM_of_JUN_OBJ_UNITS */
            (SUM(t1.JUN_OBJ_UNITS)) AS JUN_OBJ, 
          /* SUM_of_JUL_OBJ_UNITS */
            (SUM(t1.JUL_OBJ_UNITS)) AS JUL_OBJ, 
          /* SUM_of_AUG_OBJ_UNITS */
            (SUM(t1.AUG_OBJ_UNITS)) AS AUG_OBJ, 
          /* SUM_of_SEP_OBJ_UNITS */
            (SUM(t1.SEP_OBJ_UNITS)) AS SEP_OBJ, 
          /* SUM_of_OCT_OBJ_UNITS */
            (SUM(t1.OCT_OBJ_UNITS)) AS OCT_OBJ, 
          /* SUM_of_NOV_OBJ_UNITS */
            (SUM(t1.NOV_OBJ_UNITS)) AS NOV_OBJ, 
          /* SUM_of_DEC_OBJ_UNITS */
            (SUM(t1.DEC_OBJ_UNITS)) AS DEC_OBJ, 
          t1.REC_CREATE_DATE, 
          t1.REC_UPDATE_DATE
      FROM SALES.QM_TH_SALES_OBJ_MONTH t1, SALES.QM_TD_DEALER t2, SALES.QM_TD_SERIES t3
      WHERE (t1.DEALER_KEY = t2.DEALER_KEY AND t1.SERIES_KEY = t3.SERIES_KEY) AND (t1.SALES_TYPE = 'RETAIL' AND 
           t1.OBJECTIVE_YEAR > 2014 AND t1.REGION_KEY IN 
           (
           2,
           3,
           8,
           9,
           10
           ) AND t1.DEALER_KEY NOT = -2 AND t1.DEALER_CD NOT IN 
           (
           'CE555',
           'WE555',
           'SC555',
...
QUIT;
PROC SQL;
   CREATE TABLE main.SALES_OBJECT AS 
   SELECT t1.OBJECTIVE_YEAR, 
          t1.SALES_TYPE, 
          t1.SALES_PLAN_NUM, 
          t1.OBJECTIVE_TYPE, 
