...
%LET MyId=HMA03468;
%LET SalesPW = M!dex@mjz8;/*M!dex@mjz8*/
...
PROC SQL;
	...
	FROM INCENT.IDM_TH_HMF_LEASE_BOOKING t1 INNER JOIN INCENT.IDM_TD_DATE t2 ON (t1.CONTRACT_BOOKING_DATE_KEY = t2.DATE_KEY) WHERE t2.SALES_YEAR >= 2010;
QUIT;
...
FROM INCENT.IDM_TH_HMF_RETAIL_BOOKING t1 INNER JOIN INCENT.IDM_TD_DATE t2 ON (t1.CONTRACT_BOOKING_DATE_KEY = t2.DATE_KEY) WHERE t2.SALES_YEAR >= 2010 AND t1.NEW_USED_CAR_CONTRACT_CD IN ( 'Y', 'D' );
QUIT;
...
PROC SQL;
	...
	WHERE (t1.RDR_DATE_KEY = t2.DATE_KEY AND t1.INCENTIVE_SALE_TYPE_KEY = t3.INCENTIVE_SALE_TYPE_KEY) AND (t1.RDR_ENTRY_DATE >= '1Nov2009:0:0:0'dt AND t1.INCENTIVE_SALE_TYPE_KEY NOT IN ( 9, 15, 16, 14, 13, )) GROUP BY t1.VEHICLE_KEY, t1.RDR_ENTRY_DATE, t1.RDR_TYPE, t2.SALES_YEAR, t2.SALES_MONTH, t1.vin;
QUIT;
...
PROC SQL;
	CREATE TABLE WJSAS.SALESSAMPLE AS 
		SELECT t1.vk, 
			/* SALES */
	(SUM(t1.SALES)) AS SALES FROM WJSAS.ALLRDRS t1 WHERE t1.SALES_YEAR >= 2005 GROUP BY t1.vk;
QUIT;
...
PROC SQL;
	...
	INNER JOIN INCENT.IDM_TD_INCENTIVE_PROGRAM t3 ON (t2.INCENTIVE_PROGRAM_KEY = t3.INCENTIVE_PROGRAM_KEY) 
...
PROC SQL;
	...
	FROM WJSAS.SALESSAMPLE t1
		LEFT JOIN INCENT.IDM_TF_INCENTIVE_PAYMENT_STMNT t2 ON (t1.vk = t2.VEHICLE_KEY)
			INNER JOIN INCENT.IDM_TD_INCENTIVE_PROGRAM t3 ON (t2.INCENTIVE_PROGRAM_KEY = t3.INCENTIVE_PROGRAM_KEY)
			INNER JOIN INCENT.IDM_TH_VEHICLE t4 on (t1.vk = t4.VEHICLE_KEY)
...
PROC SQL;
	CREATE TABLE WJSAS.SALES AS 
		SELECT t1.RDR_ENTRY_DATE AS RDRDATE, 
			/* Sales */
	(SUM(ifn(t1.RDR_TYPE='RDR',1,-1))) AS Sales FROM INCENT.IDM_TH_RDR t1 WHERE t1.RDR_ENTRY_DATE >= '1Nov2009:0:0:0'dt AND t1.INCENTIVE_SALE_TYPE_KEY NOT IN ( 9, 15, 16, 14, 13 ) GROUP BY t1.RDR_ENTRY_DATE;
QUIT;
...
PROC SQL;
	CREATE TABLE WORK.SALES_SQUISH AS 
		SELECT t1.VEHICLE_KEY, 
			...
			t1.DEALER_KEY
		FROM INCENT.IDM_TH_RDR t1
			WHERE t1.RDR_ENTRY_DATE >= '1Nov2009:0:0:0'dt AND t1.RDR_TYPE = 'RDR'
...
PROC SQL;
	...
	;
QUIT;
data work.incentives_database00;
	...
	rename	'Dealer Cash'N = DealerCash;
		...
		rename 'Hyundai Circle A'n = Circle_A;
		rename 'Hyundai Circle DEPP'n = Circle_DEPP;
		rename 'Hyundai Circle E'n = Circle_E;
		rename 'Hyundai Circle M'n = Circle_M;
		rename 'Hyundai Circle O'n = Circle_O;
		rename 'Hyundai Circle V'n = Circle_V;
		/*rename 'Hyundai Circle B'n = Circle_B;
		/*rename 'Hyundai Circle W'n = Circle_W;*/
		rename 'Boost Up'n = Boost;
		rename 'Final Pay X'n = FinalPay_X;
		rename 'Hyundai Circle K'n = Circle_K;
		rename 'Lease Coupon'n = Lease_Coupon;
		rename 'Dealer Performance Bonus'n = DPB;
		rename 'SRC-CPO'n = SRC_CPO;
	;
run;
/* ADDDEDD August 13 2018  - This takes all the spaces out of the names and renames all the columns*/
PROC CONTENTS DATA = work.incentives_database00 OUT = work.dictionary NOPRINT ;
RUN ;
/*PROC SQL NOPRINT ;
...
PROC SQL NOPRINT ;
SELECT CATS(NLITERAL(name),"=",TRANSLATE(STRIP(TRANWRD(name,"-", "")),"_"," ")) INTO : renameList SEPARATED BY " "
FROM work.dictionary
WHERE STRIP(name) CONTAINS " "
;
quit;
PROC DATASETS LIB = work NOLIST ;
MODIFY incentives_database00 ;
RENAME &renameList ;
RUN ; QUIT ;


data wjsas.incentives_database;
	set work.incentives_database00;
/*	rename 'Base Star'N = BaseSTAR;
	rename	'Dealer Cash'N = DealerCash;
		rename	'Autoshow Rebate'N = Autoshow;
		rename	'star contribution'n =STARContrib;
...
PROC SQL;
   ...
   SELECT t4.SALES_YEAR, 
      ...
      WHERE (t1.VEHICLE_KEY = t2.VEHICLE_KEY AND t1.DEALER_KEY = t3.DEALER_KEY AND t1.RDR_DATE_KEY = t4.DATE_KEY) AND 
           (t1.RDR_ENTRY_DATE >= '1Jan2010:0:0:0'dt AND t1.INCENTIVE_SALE_TYPE_KEY NOT IN 
           (
           9,
           13,
           14,
