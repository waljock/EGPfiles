...
PROC SQL;
	...
	t2.SALES_YEAR,
	(1) as COUNTER 
	FROM INCENT.IDM_TH_HMF_LEASE_BOOKING t1 INNER JOIN INCENT.IDM_TD_DATE t2 ON (t1.CONTRACT_BOOKING_DATE_KEY = t2.DATE_KEY) 
WHERE t2.SALES_YEAR >= 2019;
QUIT;
/*PROC SQL;*/
/*	CREATE TABLE WORK.LEASE AS */
/*		SELECT t1.VEHICLE_KEY AS VK, */
...
FROM INCENT.IDM_TH_HMF_RETAIL_BOOKING t1 INNER JOIN INCENT.IDM_TD_DATE t2 ON (t1.CONTRACT_BOOKING_DATE_KEY = t2.DATE_KEY) 
WHERE t2.SALES_YEAR >= 2019 AND t1.NEW_USED_CAR_CONTRACT_CD IN ( 'Y', 'D' );
QUIT;
...
PROC SQL;
	...
	FROM INCENT.IDM_TH_RDR t1, INCENT.IDM_TD_DATE t2, INCENT.IDM_TD_INCENTIVE_SALE_TYPE t3 
		WHERE (t1.RDR_DATE_KEY = t2.DATE_KEY AND t1.INCENTIVE_SALE_TYPE_KEY = t3.INCENTIVE_SALE_TYPE_KEY) 
		AND (t1.RDR_ENTRY_DATE >= '3Jan2019:0:0:0'dt AND t1.INCENTIVE_SALE_TYPE_KEY NOT IN ( 9, 15, 16, 14, 13 )) 	
	GROUP BY t1.VEHICLE_KEY, t1.DEALER_KEY, t1.RDR_ENTRY_DATE, t1.RDR_TYPE, t2.SALES_YEAR, t2.SALES_MONTH, t1.vin;
QUIT;
...
proc sort data=wjsas.allrdrs (keep=vk vin) noduprecs out=work.convert1;
	by vk;
run;
...
proc sort data=work.collgrad NODUPRECS out=WORK.ALLCOLL;
	by vk;
run;

proc sort data=WORK.ALLHMF NODUPRECs out=WORK.HMFNODUP;
	by vk conDate;
	where vk <> -1;
	where lastcon = 1;
run;
...
PROC SQL;
	CREATE TABLE WJSAS.SALESSAMPLE AS 
		SELECT t1.vk, 
			/* SALES */
	(SUM(t1.SALES)) AS SALES FROM WJSAS.ALLRDRS t1 WHERE t1.SALES_YEAR >= 2019 GROUP BY t1.vk;
QUIT;
...
PROC SQL;
	CREATE TABLE WJSAS.SALES AS 
		SELECT t1.RDR_ENTRY_DATE AS RDRDATE, 
			/* Sales */
	(SUM(ifn(t1.RDR_TYPE='RDR',1,-1))) AS Sales FROM INCENT.IDM_TH_RDR t1 WHERE t1.RDR_ENTRY_DATE >= '1Nov2012:0:0:0'dt AND t1.INCENTIVE_SALE_TYPE_KEY NOT IN ( 9, 15, 16, 14, 13 ) GROUP BY t1.RDR_ENTRY_DATE;
QUIT;
