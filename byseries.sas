PROC SQL;
   ...
   SELECT DISTINCT t1.SALES_YEAR, 
      ...
      WHERE (t1.MODEL_YEAR = t2.MODEL_YEAR AND t1.MODEL_CD = t2.MODEL_CD) AND (/*t1.ADI_GROUP = 'N' AND */
           t1.MODEL_YEAR = 2019) and t1.ADI_GROUP ne 'OTH' and t1.SERIES_CD in( 'Q', 'J', '2','4','6','8');
QUIT;
...
proc gchart data=work.trim_graph;
  vbar SERIES_CD / sumvar= WHOLESALE_RETAIL_DIFF /* group=*/
...
TITLE "Mix By Model";
  vbar SERIES_CD / sumvar= totalincentives /* group=*/
...
proc sql;
	create table survival_summary2a as
		select 
			...
			,count(*) as cnt
		from survival2t
		group by SERIES_CD, WHOLESALE_RETAIL_DIFF, WHOLESALE_DATE
	;
quit;
data sur_summ2a;
...
proc sql;
	create table survival_summary2a as
		...
		order by SERIES_CD, WHOLESALE_RETAIL_DIFF, WHOLESALE_DATE
	;
quit;
...
PLOT hazard * WHOLESALE_RETAIL_DIFF =SERIES_CD	
 /
 	VAXIS=AXIS1
	HAXIS=AXIS2
